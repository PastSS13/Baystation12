...
mob/living/InCone(mob/center = usr, dir = NORTH)
	. = ..()
	for(var/obj/item/grab/O in center)//TG doesn't have the grab item. But if you're porting it and you do then uncomment this.
		if(src == O.affecting)
			return 0
		else
			return .
...
mob/living/update_vision_cone()
	...
	if(src.client && src.fov)
		...
		if(fov.alpha != 0)
			...
			var/obj/effect/decal/cleanable/blood/O
